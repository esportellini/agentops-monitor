...
export default function RunsPage() {
  ...
  return <ProtectedLayout><div className="p-8 max-w-6xl">
    ...
    {showForm && <div className="mb-6 space-y-4 rounded-lg border border-surface-border bg-surface-card p-5">
      ...
      {provider === "openai" && <div className="rounded-md border border-yellow-400/30 bg-yellow-400/5 p-3 text-sm text-text-secondary"><p>This evaluation sends case inputs to the selected external model provider.</p><label className="mt-2 flex items-start gap-2 text-xs text-text-primary"><input type="checkbox" checked={consent} onChange={event => setConsent(event.target.checked)} className="mt-0.5" />I understand and allow sending this evaluation dataset to the external provider.</label></div>}
